...
class CloudKitManager {
    ...
    func getICloudAccountStatus() async throws -> CKAccountStatus {
        ...
        logger.debug("Icloud account status fetched")
        return ret
    }
    ...
    func userRecordID(completionHandler: @escaping (Result<CKRecord.ID, Error>) -> Void) {
        container.fetchUserRecordID { recordID, error in
            if let recordID = recordID {
                self.logger.debug("User record id fetched success")
                completionHandler(.success(recordID))
            } else if let error = error {
                self.logger.debug("User record id fetched failure")
    ...
    func userRecordID() async throws -> CKRecord.ID {
        ...
        self.logger.debug("User record id fetched")
        return ret
    }
    ...
    func userRecord() async throws -> CKRecord {
        let ret = try await findRecord(for: userRecordID())
        self.logger.debug("User record CKRecord fetched")
        return ret
    }
    ...
    func records(for ids: [CKRecord.ID], desiredKeys: [CKRecord.FieldKey]? = nil) async throws -> [CKRecord.ID: CKRecord] {
        ...
        let ret = try processResults(results)
        self.logger.debug("Fetch record async")
        return ret
    }
    ...
    func findRecord(for recordID: CKRecord.ID) async throws -> CKRecord {
        let ret = try await database.record(for: recordID)
        self.logger.debug("Find record async")
        return ret
    }
    ...
    func findRecords(matching query: CKQuery, inZoneWith zoneID: CKRecordZone.ID? = nil, desiredKeys: [CKRecord.FieldKey]? = nil, resultsLimit: Int = CKQueryOperation.maximumResults) async throws -> ([CKRecord], CKQueryOperation.Cursor?) {
        ...
        self.logger.debug("Find records matching async")
    ...
    func saveRecord(saving recordToSave: CloudKitConvertible) async throws -> CKRecord {
        ...
        if let savedRecord = saveResults.values.first {
            self.logger.debug("Record saved async")
            return savedRecord
        } else {
    ...
    func deleteRecord(deleting recordIDToDelete: CKRecord.ID) async throws {
        ...
        self.logger.debug("Record deleted async")
    }
